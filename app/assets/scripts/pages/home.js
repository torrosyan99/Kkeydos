...
if (featuredSlider) {
  new Swiper(featuredSlider.querySelector('[data-featured-slider]'), {
    speed: 650,
    slidesPerView: 1,
    spaceBetween: 0,
    loop: true,
    ...
    navigation: {
      prevEl: featuredSlider.querySelector('[data-featured-prev]'),
      nextEl: featuredSlider.querySelector('[data-featured-next]'),
    },
    pagination: {
      el: featuredSlider.querySelector('[data-featured-pagination]'),
      clickable: true,
    },
  });
}
...
if (testimonialsSlider) {
  new Swiper(testimonialsSlider.querySelector('[data-testimonials-slider]'), {
    speed: 650,
    slidesPerView: 'auto',
    slidesPerGroup: 1,
    spaceBetween: 32,
    ...
    pagination: {
      el: testimonialsSlider.querySelector('[data-featured-pagination]'),
      clickable: true,
    },
    breakpoints: {
      834: {
...
const initTeamAssembly = () => {
  const section = document.getElementById('team-assembly');
  ...
  const members = order
    .map((slot) => visual.querySelector(`[data-team-member][data-team-slot="${slot}"]`))
    .filter(Boolean);
  ...
  const elapsed = () =>
    Math.max(0, ...active.map(({ animation }) => Number(animation.currentTime) || 0));
  ...
  const measure = () => {
    const path = tracks.find((item) => getComputedStyle(item.ownerSVGElement).display !== 'none');
    if (!path) return null;
  ...
  const makeAnimation = (element, frames, options, time) => {
    ...
    const end =
      options.iterations === Infinity ? Infinity : (options.delay || 0) + options.duration;
  ...
  const rebuild = () => {
    ...
    try {
      ...
      const previous =
        stage === 'static' ? resume || { stage: 'ready', time: 0 } : { stage, time: elapsed() };
      resume = null;
      ...
      const assemblyEnd =
        settings.rowHold + settings.assembly + (members.length - 1) * settings.stagger;
...
if (insightsSlider) {
  new Swiper(insightsSlider.querySelector('[data-insights-slider]'), {
    speed: 500,
    ...
    spaceBetween: 20,
    navigation: {
      prevEl: insightsSlider.querySelector('[data-insights-prev]'),
      nextEl: insightsSlider.querySelector('[data-insights-next]'),
    },
    breakpoints: {
      640: {
        slidesPerView: 2,
